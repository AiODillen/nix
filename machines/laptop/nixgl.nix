...
# (vars.gpu), so both gpu.nix and niri.nix import this to stay in sync.
#
...
let
  ...
  glName = if vars.gpu == "nvidia" then "nixGLNvidia" else "nixGLIntel";
  vulkanName = if vars.gpu == "nvidia" then "nixVulkanNvidia" else "nixVulkanIntel";
  gl = nixgl.${glName};
